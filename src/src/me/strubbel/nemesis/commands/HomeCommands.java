...
public class HomeCommands {
  ...
  @Command(name = "sethome", usage = "/sethome <name>", permission = "nemesis.home",
      inGameOnly = true)
  public void setHome(CommandArgs args) {
    ...
    if (args.length() > 0) {
      ...
      p.sendMessage(Util.getPrefix() + " §7Gib einen Namen an.");
  }


  @Command(name = "delhome", usage = "/delhome <name>", permission = "nemesis.home",
      inGameOnly = true)
  ...
  public void home(CommandArgs args) {
    ...
    if (args.length() > 0) {
      ...
      if (cfg.contains("homes." + name.toLowerCase())) {
        World w =
            Bukkit.getServer().getWorld(cfg.getString("homes." + name.toLowerCase() + ".world"));
        ...
        p.sendMessage(Util.getPrefix() + " §7Du wurdest zum Home §6"
            + cfg.get("homes." + name.toLowerCase() + ".name") + " §7teleportiert.");
      } else
        p.sendMessage(Util.getPrefix() + " §7Dieses Home existiert nicht.");
    } else
      p.sendMessage(Util.getPrefix() + " §7Gib einen Namen an.");
  }
