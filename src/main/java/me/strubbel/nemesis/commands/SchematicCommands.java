...
public class SchematicCommands {
    ...
    public void schemLoad(CommandArgs args) {
		if (!args.getArgs(0).toLowerCase().startsWith("pr/")) {
			...
			if ((!(args.getPlayer().hasPermission("nemesis.schematic.member") && args.getPlayer().hasPermission("nemesis.schematic.pr"))) || (!args.getPlayer().isOp())) {
				if (args.length() > 0) {
	...
	public void schemDelete(CommandArgs args) {
		...
		if (!args.getArgs(0).toLowerCase().startsWith("pr/")) {
			if ((!(args.getPlayer().hasPermission("nemesis.schematic.member")
					&& args.getPlayer().hasPermission("nemesis.schematic.pr"))) || (!args.getPlayer().isOp())) {
				if (args.length() > 0) {
					if ((!(args.getPlayer().hasPermission("nemesis.schematic.member")
							&& args.getPlayer().hasPermission("nemesis.schematic.pr"))) || (!args.getPlayer().isOp())) {
						if (args.length() > 0) {
