...
public class GamemodeCommand {
  @Command(name = "gamemode", aliases = "gm", usage = "/gamemode [player]",
      permission = "nemesis.gamemode", inGameOnly = true)
      ...
      case 2:
        if (Bukkit.getPlayerExact(args.getArgs(1)) != null) {
          ...
          if (getGameMode(p, args.getArgs(0)) != null) {
            ...
            target.sendMessage(Util.getPrefix() + " §7Dein Spielmodus wurde von §6"
                + p.getDisplayName() + " §7geändert.");
            p.sendMessage(Util.getPrefix() + " §7Der Spielmodus von §6" + target.getDisplayName()
                + " §7wurde geändert.");
          }
        } else
