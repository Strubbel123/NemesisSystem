...
public class TeleportCommands {
  ...
  @Command(name = "tphere", aliases = "s", usage = "/tphere <player>", permission = "nemesis.tp",
      inGameOnly = true)
  public void tphere(CommandArgs args) {
    ...
    if (args.length() > 0) {
      if (Bukkit.getPlayerExact(args.getArgs(0)) != null) {
        ...
        target.teleport(p);
        target.sendMessage(
            Util.getPrefix() + " §7Du wurdest zu §6" + p.getDisplayName() + " §7teleportiert.");

        p.sendMessage(
            Util.getPrefix() + " §6" + target.getDisplayName() + " §7wurde zu dir teleportiert.");
      } else
        p.sendMessage(Util.getPrefix() + " §7Dieser Spieler existiert nicht.");
    } else
      p.sendMessage(Util.getPrefix() + " §7Gib einen Spieler an.");
  }
  ...
  public void tpall(CommandArgs args) {
    ...
    Bukkit.broadcastMessage(Util.getPrefix() + " §7Alle Spieler wurden zu §6" + p.getDisplayName()
        + " §7teleportiert.");
  }
}
